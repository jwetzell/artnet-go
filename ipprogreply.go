...
func (ad *ArtIpProgReply) GetOpCode() uint16 {
	return ad.OpCode
}

func (ad *ArtIpProgReply) GetProtVer() uint16 {
	return uint16(ad.ProtVerHi)<<8 + uint16(ad.ProtVerLo)
}

func (ad *ArtIpProgReply) GetID() [8]uint8 {
	return ad.ID
}

func (ad *ArtIpProgReply) UnmarshalBinary(data []byte) error {
	if len(data) < 34 {
	...
	ad.Status = data[offset+14]
	ad.spare2 = data[offset+15]

	ad.ProgDgHi = data[offset+16]
	ad.ProgDg2 = data[offset+17]
	ad.ProgDg1 = data[offset+18]
	ad.ProgDgLo = data[offset+19]

	ad.spare7 = data[offset+20]
	ad.spare8 = data[offset+21]

	return nil
}

func (ad *ArtIpProgReply) MarshalBinary() ([]byte, error) {
	data := make([]byte, 8+26)
	copy(data[0:8], ad.ID[:])
